    ...
    <tbody>
		   ...
		   if(isset($value['child'])){
			...
			///if(!empty($value['child']))  {
				...
				?>
				<tr>
                <?php $name=mysql_real_escape_string(trim($ci['name']));
			 ...
			 if(in_array($name,$arrname)) 
				{
				?>
                <td><a href="<?php echo site_url('/administrator/Reauthorization/get_report/'.$serial_no).'/'.base64_encode($file_no).'/'.$salutation;?>" target="_blank" class="open-dialog-edit btn btn-success btn-rad" data-id="<?php //echo $list['Branch_Code']; ?>"><i class="icon-pencil"></i>Print</a></td>
                <?
                }
				}else{?>
