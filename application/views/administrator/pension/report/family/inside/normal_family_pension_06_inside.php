...
if($pensioner->dor==$pensioner->dod){
    ...
    if($cnt=="3"){
    	...
    	$enhance_from_upto="<b>E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('Y-m-d')." to ".$enhan_upto->format('Y-m-d')."</b>";
    }else if($cnt=="4"){
    ...
    //$enhance_from_upto="<b>E/R of Rs.".$enhance_from_upt."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('Y-m-d')." to ".$enhan_upto->format('Y-m-d')."</b>";
	...
	$sanjay_enhance=$enhan_upto->format('Y-m-d');

	$dor = new DateTime($pensioner->dor);
	$dor->modify('+1 day');

    $enhance_from_upto="Life time arrear pension Rs ".$pensioner->getEnhanceRate_sanjay(false)."<b> w.e.f. ".$dor->format('Y-m-d')." to ".$pensioner->dod."<b> and thereafter E/R of Rs.".$pensioner->getEnhanceRate_sanjay(false)."/- PM".$pensioner->getDRMA()." w.e.f ".$enhan_from->format('Y-m-d')." to ".$enhan_upto->format('Y-m-d')."</b>";
	...
	<div id="print1" style="width: 1000px; margin: 0px auto;">
		...
		3.  (a) Adhoc/Temporary increase
		    ...
		    (d) Family Pension in case of death of the Pensioner 	:-<b><?php echo $amount_pension;?></b>
			  i) Enhanced rate 						:- <b>Rs. <?php echo $pensioner->getEnhanceRate_sanjay(false); ?> from <?php echo $enhan_from->format('Y-m-d')?> upto <?php echo $sanjay_enhance;//$enhan_upto->format('Y-m-d');?></b>
						</pre>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<?php elseif($pensioner->dod == $pensioner->dor) : ?>
						<td style="vertical-align: top;" colspan="4"><br /><?php echo nbs(14); ?>Date of commencement of pension :- <b><?php echo $pensioner->dateTimeToDate($pensioner->effect_of_pension); ?></b></td>
					<?php endif; ?>
				</tr>
				<tr>
					<td style="vertical-align: top;" colspan="4">
				...
				<tr>
					<td style="vertical-align: top;" colspan="4">
						...
						1) <b><?php echo strtoupper($pensioner->salutation." ".$pensioner->name).',<br />'.nbs(5).str_replace(",", ",<br />".nbs(4), $pensioner->address_after_retirement); ?></b><br /><br />
						2) <b><?php echo str_replace(",", ",<br />".nbs(5), $pensioner->office_address); ?></b><br />
...
<div id="disburser_disburser_portion" style="display: none;">
					...
					<td style="vertical-align: top;" colspan="4">
						<table border="1" cellspacing="2" cellpadding="2" width="100%" id="inner-table">
							...
							<tr>
								...
								<td style="vertical-align: top; text-align: center;" width="20%"><b><?php echo $pensioner->getDOBofSpouse(); ?></b></td>
								<td style="vertical-align: top; text-align: center;" width="20%"><b><?php echo $pensioner->religion.", ".$pensioner->nationality;?></b></td>
								<td style="vertical-align: top; text-align: center;" width="20%"><b><?php echo $pensioner->address_after_retirement; ?></b></td>
								<td style="vertical-align: top; text-align: center;" width="20%"><b><?php echo $enhance_from_upto;?> and ordinary rate <?php echo $pensioner->getOrdinaryRate(); ?>/-PM<?php echo $pensioner->getDRMA(); ?> w.e.f <?php echo $ordinary_from_upto;?></b></td>
							</tr>
						</table>
					</td>
				</tr>
					...
					<td style="vertical-align: top; text-align: justify;" colspan="4">
						<?php
							...
							if(in_array($class_of_pension, $array)) {
								echo nbs(6);echo  'UNTILL FURTHER NOTICE and on the expiration of every month, be pleased to pay to '.strtoupper($pensioner->salutation." ".$pensioner->name).' Rtd '.$pensioner->designation.'</b>, a sum of <b>Rs.'.$pensioner->getAmountofPension().'/-</b>PM'.$pensioner->getDRMA().' w.e.f  only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
	...
	echo nbs(6);echo 'UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to '.strtoupper($spouse).'('.strtoupper($spouse_type).')'.' of <b>Late '.$pensioner->name.' Ex- '.$pensioner->designation.'</b>, a sum of <b>Rs.'.$pensioner->getEnhanceRate_sanjay(false).'/-</b>PM'.$pensioner->getDRMA().' './*str_replace(['E/R', ''], '', $enhance_from_upto).*/' only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
	The payment should commence from '.date_format(date_create($pensioner->effect_of_pension), "d-m-Y").'</b>.<br/>&nbsp';
...
<div id="disburser_pensioner_portion" style="display: none;">
	...
	echo nbs(6);echo 'UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to '.strtoupper($spouse).'('.strtoupper($spouse_type).')'.' of <b>Late '.$pensioner->name.' Ex- '.$pensioner->designation.'</b>, a sum of <b>Rs.'.$pensioner->getEnhanceRate_sanjay(false).'/-</b>PM'.$pensioner->getDRMA().' './*$enhance_from_upto.*/' only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
	The payment should commence from '.date_format(date_create($pensioner->effect_of_pension), "d-m-Y").'</b>.<br/>&nbsp';
							}
						?>
			 		</td>
