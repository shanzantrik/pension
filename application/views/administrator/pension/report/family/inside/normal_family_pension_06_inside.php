...
/**/
	...
	$child_age="0000-00-00";
	if($pensioner->dor==$pensioner->dod){
		$child_age=$pensioner->getDOBOfLegalHeirX(1);
	}else{
		$child_age=$pensioner->getDOBOfLegalHeirNameSingleWife();//dob;
	}

	$child_age_plus_25=new DateTime($child_age);
	$child_age=new DateTime($child_age);
	
	$child_age_plus_25=$child_age_plus_25->modify('+25 year');
	//;
	//echo $child_age->format('Y-m-d');

	//seven pay
	$ordinary_from_seven=$enhan_upto_seven->modify('+1 day');
	//echo $ordinary_from_seven->format('Y-m-d');
	$tage=$child_age->diff($ordinary_from_seven);
	//echo //$tage->y;
	//echo $ordinary_from_seven->format('Y-m-d');
	$isLegalHeirCase=$pensioner->getIsLegalHeirCase();
	if($isLegalHeirCase==1){
		if($tage->y<=25){
			$child_age->modify('+25 years');
			$ordinary_from_upto_seven="<b> ".$ordinary_from_seven->format('d-m-Y')." to ".$child_age->format('d-m-Y')." until earns livelihood or gets married"."</b>";
		}
	}else{
		...
		$ordinary_from_upto_seven_below25="<b> ".$ordinary_from_seven->format('d-m-Y')." to ".$child_age_plus_25->format('d-m-Y')." or earns livelihood or gets married"."</b>";
	}
	...
	$life_time_from_upto="";
		...
		//seven pay
		$enhan_upto_seven= new DateTime($pensioner->dor);
	...
	/**/
		...
		if($cnt=="3" && $pensioner->pay_commission==7){
			...
			$enhance_from_upto="<b>Life time arrear pension @ Rs. ".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".date_format(date_create($pensioner->dor),"d-m-Y")." to ".date_format(date_create($pensioner->dod),"d-m-Y").".<br> E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$enhan_upto_seven->format('d-m-Y')."</b>";
		}else if($cnt=="4" && $pensioner->pay_commission==7){
			$return = round($x_amount*34/100);
			$enhance_from_upto="<b>Life time arrear pension @ Rs. ".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".date_format(date_create($pensioner->dor),"d-m-Y")." to ".date_format(date_create($pensioner->dod),"d-m-Y").".<br> E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$enhan_upto_seven->format('d-m-Y')."</b>";
		}elseif($pensioner->pay_commission==7){
			...
			$dwef_from= new DateTime($pensioner->dor);
			$dwef_from->modify('+1 day');
			...
			if($age>=2018 || $age==0)
			{
				...
				//555
				$enhance_from_upto="<b>Life time arrear pension @ Rs. ".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$dwef_from->format('d-m-Y')." to ".date_format(date_create($pensioner->dod),"d-m-Y").".<br> E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$enhan_upto_seven->format('d-m-Y')."</b>";
			}
			elseif($age<25)
			{
				//$enhance_from_upto="<b>E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$pensioner->getPensionEndDate()." thereafter no family pension</b>";
								...
								$differ = $from->diff($upto)->format("%y");
								   ...
								   {
									   //$return.= " <b>from ".$from->format('d-m-Y')." upto ".$upto->format('d-m-Y')." therefore no family pension</b>";
									   $enhance_from_upto="<b>Life time arrear pension @ Rs. ".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$dwef_from->format('d-m-Y')." to ".date_format(date_create($pensioner->dod),"d-m-Y").".<br> E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$pensioner->getPensionEndDate()." until earns livelihood or gets married</b>";//therefore no family pension
								   }
								   elseif($differ>10)
								   {
									   $uptonew = new DateTime($pensioner->dor);
									   ...
									   //$return.= " <b>from ".$from->format('d-m-Y')." upto ".$uptonew->format('d-m-Y')."</b>";
									   $enhance_from_upto="<b>Life time arrear pension @ Rs. ".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".date_format(date_create($pensioner->dor),"d-m-Y")." to ".date_format(date_create($pensioner->dod),"d-m-Y").".<br> E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$from->format('d-m-Y')." to ".$uptonew->format('d-m-Y')."</b>";
								   }
			}
			elseif($age>=25)
			{
				//$enhance_from_upto="<b>E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$enhan_upto_seven->format('d-m-Y')."</b>";
				$enhance_from_upto="<b>Life time arrear pension @ Rs. ".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".date_format(date_create($pensioner->dor),"d-m-Y")." to ".date_format(date_create($pensioner->dod),"d-m-Y").".<br> E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$pensioner->getPensionEndDate()." thereafter no family pension</b>";
			}
		...
		$ordinary_from_upto_seven="<b> ".$ordinary_from_seven->format('d-m-Y')." until his/her remarriage or death"."</b>";
		$ordinary_from_upto_seven_above25="<b> ".$ordinary_from_seven->format('d-m-Y')." until earns livelihood or gets married"."</b>";
		$ordinary_from_upto_seven_below25="<b> ".$ordinary_from_seven->format('d-m-Y')." to ".$date->format('d-m-Y')." or earns livelihood or gets married"."</b>";
		//seven pay
			 	...
			 	<tr>
					<td style="vertical-align: top;" colspan="4" align="left"><?php echo nbs(13); ?>
					  ...
					  ?></b> in favour of <b><?php 
								$sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					   if($NumberOfWives==1){
						$new_sentance=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)." Ex. ".$pensioner->designation." through the Legal guardianship of ";						
						echo $pensioner->getNameOfLegalHeirCorrection_SingleWife(true,$new_sentance,$sentence_of_lifetime_arear);//getNameOfLegalHeir();
						//echo $pensioner->getNameOfLegalHeir();	
					}else{
						echo $pensioner->getNameOfLegalHeirX($i+1).$sentence_of_lifetime_arear;
					}?>
							</b> and to request that the pensioner’s portion of the PPO may be made over to him/her after obtaining his/her signature on the disburser’s portion after you have satisfied yourself of his/her identity and payments noted in both the portion as they are made. The slip bearing the left/right hand thumb impressions of the pensioners is also enclosed.</p>
					  <p align="justify">
				...
				<tr>
					<td style="vertical-align: top;" colspan="4">
						...
						<b>1) 
								<?php 
								if($NumberOfWives==1){
									echo $pensioner->getNameOfLegalHeirNameSingle();//getNameOfLegalHeir();	
								}else{
...
<div style="width:1000px; min-height:600px; font-size: 1.2em; color:#000000; background-color:#FFFFFF; line-height: 1.5em">
		<!-- <div style="width:1000px; min-height:600px; padding: 0 20px; font-size: 1.1em; color:#000000; background-color:#FFFFFF; line-height: 2em"> -->
			...
			<table width="100%" border="0" cellpadding="2" id="report">
				...
				<tr>
					<td colspan="4"><div align="left" style="text-align: justify;"><?php echo nbs(6); ?> I am to request that you will be so good as to arrange for payment a sum of <b>Rs. <?php //if($pensioner->pay_commission==7){ echo $bincs;} else{ echo $pensioner->getDCRG();} 
					 ...
					 if($pensioner->dod==$pensioner->dor){
						...
						echo $pensioner->getDCRG_diff_dod_dor()/$NumberOfWives;
					}
					...
					$sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					   if($NumberOfWives==1){
						...
						echo $pensioner->getNameOfLegalHeirX($i+1).$sentence_of_lifetime_arear;
					}?> </b> <?php  ?> being the amount of retirement/death gratuity sanctioned to him/her in letter no. <b><?php echo $gpo_no; ?></b> from the Director of Audit & Pension, debitable to 2071 Pension & ORB etc.</div></td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="4" style="padding: 10px 0;"><div align="left" style="text-align: justify;">i. <b><?php $sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					if($NumberOfWives==1){
					 $new_sentance=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)." Ex. ".$pensioner->designation." through the Legal guardianship of ";						
					 echo $pensioner->getNameOfLegalHeirCorrection_SingleWife(true,$new_sentance,$sentence_of_lifetime_arear);//getNameOfLegalHeir();
					 //echo $pensioner->getNameOfLegalHeir();	
	...
	<div id="print3<?php echo $i+1 ?>" style="width: 1000px; margin: 0px auto;">
		...
		<div style="width:1000px; min-height:600px; font-size:1.2em; color:#000000; background-color:#FFFFFF; line-height: 1.5em">
			...
			<table width="100%" border="0" cellpadding="2" id="report">
				...
				<?php
				$binaz2=substr($pensioner->total_service,0,2);
				//echo $bina;
				$binbz2=$pensioner->total_amount;
				$binyz2=substr($pensioner->total_service,9,1);
				$binxz2=$pensioner->smp;
				//echo $biny;
				if($binaz2 > 33)
				{
				 $bincz2=($binbz2)*(1/4)*(33*2);
					//$binc=1;
				}
				else if($binaz2 < 33 && $binyz2 < 9)
				{
					//$binc=2;
					$bincz2=($binbz2*1/2)*($binxz2);
				...
				else
				{
					...
					//$binc=0;
					$bincz2=($binbz2*2/100+$binbz2)*(1/4)*($binxz2);
				}
				?>
				<?php
					$pensioner_amount_final=0;
					$age_retires2=$pensioner->age_retire+1;
					//echo $age_retire;
					$result2 = mysql_query("SELECT distinct comm_value FROM master_comm_value_tb where Age_Next_Birth='".$age_retires2."' LIMIT 1");
					if($row = mysql_fetch_array($result2))
					{
						$vivos2=$row['comm_value'];
						$bigbazars2=ceil(($total_amount)*50/100*0.4*$vivos2*12);
						$visals2=($total_amount)*50/100*40/100;
						$sohums2=($total_amount)*50/100;
					}
					//aj123
					?>
				<tr>
					<td colspan="4"><div align="left" style="text-align: justify;"><?php echo nbs(6); ?>I am to request you to make necessary arrangement for payment of <b>Rs.<?php if($pensioner->pay_commission==7){ echo $bigbazars2;} else{echo $pensioner->getCommutationOfPension();} ?>/- (Rupees <?php 
						if($pensioner->pay_commission==7){ echo no_to_words($bigbazars2);} else{echo no_to_words($pensioner->getCommutationOfPension());} 
						?>)</b> being the commuted value of <b>Rs. <?php if($pensioner->pay_commission==7){ echo $bigbazars2;} else{echo $pensioner->getCommutationOfPension();} ?>/- (Rupees <?php 
						if($pensioner->pay_commission==7){ echo no_to_words($bigbazars2);} else{echo no_to_words($pensioner->getCommutationOfPension());} ?>)</b> out of pension of <b>Rs. <?php echo $pensioner->getEnhanceRate_sanjay(false); ?>/- (Rupees <?php echo no_to_words($pensioner->getEnhanceRate_sanjay(false));?>)</b> per month granted to <b> <?php 
								if($NumberOfWives==1){
									echo $pensioner->getNameOfLegalHeir();	
								}else{
									echo $pensioner->getNameOfLegalHeirX($i+1);
		...
		<div style="width:1000px; min-height:600px; font-size: 1.2em; color:#000000; background-color:#FFFFFF; line-height: 1.5em">
			...
			<table width="100%" border="0" cellpadding="2" id="report">
				...
				<tr>
					...
					<td colspan="2" valign="top" style="padding: 10px 0;"><b><?php $sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					   if($NumberOfWives==1){
						$new_sentance=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)." Ex. ".$pensioner->designation." through the Legal guardianship of ";						
						echo $pensioner->getNameOfLegalHeirCorrection_SingleWife(true,$new_sentance,$sentence_of_lifetime_arear);//getNameOfLegalHeir();
						//echo $pensioner->getNameOfLegalHeir();	
				...
				<?php
					...
					<td colspan="2" valign="top" style="padding: 10px 0;">
		                ...
		                if($pensioner->dod==$pensioner->dor){
							...
							echo $pensioner->getDCRG_diff_dod_dor();
						}
		                ?>/-</b></br>
		                <b>COMMUTATION RS. <?php 
		                if($pensioner->com_applied == "1")
						{
							 if($pensioner->pay_commission==7 && $pensioner->dor==$pensioner->dod){ echo 'N/A';}elseif($pensioner->pay_commission==7){ echo $bigbazars;} else{echo $pensioner->getCommutationOfPension();}
						}else{
							echo 'N/A';
						}
...
<div id="disburser_disburser_portion<?php echo $i+1; ?>" style="display: none;">
				...
				<tr>
			 	...
			 	<tr>
					<td style="vertical-align: top;" colspan="2">
						...
						<div style="float: left;"><b><?php $sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					   if($NumberOfWives==1){
						$new_sentance=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)." Ex. ".$pensioner->designation." through the Legal guardianship of ";						
						echo $pensioner->getNameOfLegalHeirCorrection_SingleWife(true,$new_sentance,$sentence_of_lifetime_arear);//getNameOfLegalHeir();
						//echo $pensioner->getNameOfLegalHeir();	
					...
					<td style="vertical-align: top;" colspan="4">
						<table border="1" cellspacing="2" cellpadding="2" width="100%" id="inner-table">
							...
							<tr>
								...
								if($pensioner->dor==$pensioner->dod){
									echo $pensioner->getDOBOfLegalHeirX($i+1);
								}else{
									echo $pensioner->getDOBOfLegalHeirNameSingleWife();//dob;
								}
								...
								<td style="vertical-align: top; text-align: center;font-size: 11px;" width="20%"><b>
								<?php 
	...
	echo nbs(6);?> UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to <b> <?php $sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					   if($NumberOfWives==1){
						$new_sentance=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)." Ex. ".$pensioner->designation." through the Legal guardianship of ";						
						echo $pensioner->getNameOfLegalHeirCorrection_SingleWife(true,$new_sentance,$sentence_of_lifetime_arear);//getNameOfLegalHeir();
						//echo $pensioner->getNameOfLegalHeir();	
	...
	The payment should commence from './*date_format(date_create($pensioner->effect_of_pension), "d-m-Y")*/$pensionDate.'</b>.&nbsp';//strtoupper($spouse).'('.strtoupper($spouse_type).')'.' of Late '.$pensioner->name.' Ex-'.$pensioner->designation
				...
				<tr>
			 	...
			 	<tr>
					<td style="vertical-align: top;font-size: 11px;" colspan="3"><b>
						To<br/>
						<?php
echo $ac=str_replace(", ", ",<br />", $pensioner->getAG($i)).str_replace(", ", ",<br />", $pensioner->getTg($i));
						 /*$tname=explode(",",$ac);
							 $ac=trim($tname[0]).''.trim($tname[1]);
							 if($pensioner->getSub_To($i)!=""){
							 	echo $pensioner->getSub_To($i);
							 }else{
							 	echo $ac;
							 }*/
						 ?></b>
...
<div id="disburser_pensioner_portion<?php echo $i+1; ?>" style="display: none;">
				...
				<tr>
			 	...
			 	<tr>
					<td style="vertical-align: top;" colspan="2">
						...
						<?php $sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					   if($NumberOfWives==1){
						$new_sentance=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)." Ex. ".$pensioner->designation." through the Legal guardianship of ";						
						echo $pensioner->getNameOfLegalHeirCorrection_SingleWife(true,$new_sentance,$sentence_of_lifetime_arear);//getNameOfLegalHeir();
						//echo $pensioner->getNameOfLegalHeir();	
	...
	echo nbs(6); ?> UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to <b><?php  $sentence_of_lifetime_arear=" of ".nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name).",".nbs(1)."Ex.".$pensioner->designation;
					 ...
					 ?><?php echo '</b>, a sum of <b>Rs.'.$orrate/$NumberOfWives.'/-</b>PM'.$pensioner->getDRMA().' './*$enhance_from_upto.*/' only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
	The payment should commence from './*date_format(date_create($pensioner->effect_of_pension), "d-m-Y")*/$pensionDate.'</b>.&nbsp';//strtoupper($spouse).'('.strtoupper($spouse_type).')'.' of Late '.$pensioner->name.' Ex-'.$pensioner->designation
							}
						?>
			 		</td>
				...
				<tr>
			 	...
			 	<tr>
					<td style="vertical-align: top;font-size: 11px;" colspan="3">
						<b>
						To<br/>
						
						<?php //$tname=explode(",",$ac);
							// $ac=$tname[0].'<br>'.$tname[1];
							 echo $ac=str_replace(", ", ",<br />", $pensioner->getAG($i)).str_replace(", ", ",<br />", $pensioner->getTg($i));
							 /*if($pensioner->getSub_To($i)!=""){
							 	echo $pensioner->getSub_To($i);
							 }else{
