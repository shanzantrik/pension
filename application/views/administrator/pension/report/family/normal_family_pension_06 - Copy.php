				...
				<td style="vertical-align: top;" colspan="4">
					<table border="1" cellspacing="2" cellpadding="2" id="inner-table">
						...
						<tr>
							...
							<td style="vertical-align: top; text-align: center;" width="20%"><?php echo $pensioner->dateTimeToDate($pensioner->getDOBofSpouse()); ?></td>
							...
							<td style="vertical-align: top; text-align: center;" width="20%">F/P of Rs.<?php echo $pensioner->getEnhanceRate_sanjay();?>/- PM+DR+MA w.e.f <?php echo $enhance_from_upto;?> and ordinary rate <?php echo $pensioner->getOrdinaryRate(); ?>/-PM+DR+MA w.e.f <?php echo $ordinary_from_upto;?></td>
						</tr>
					</table>
				</td>
			</tr>
				...
				<td style="vertical-align: top;" colspan="4">
					...
					<?php
						...
						if(in_array($pensioner->class_of_pension, $array)) {
							echo nbs(6);echo  'UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to '.strtoupper($pensioner->salutation." ".$pensioner->name).' Rtd '.$pensioner->designation.'</b>, a sum of <b>Rs.'.$pensioner->getAmountofPension().'/-</b> only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $pensioner->class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
...
echo nbs(6);echo 'UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to '.strtoupper($spouse).'('.strtoupper($spouse_type).')'.' of <b>Late '.$pensioner->name.' '.$pensioner->designation.'</b>, a sum of <b>Rs.'.$pensioner->getEnhanceRate_sanjay().'/-</b> only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $pensioner->class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
The payment should commence from '.date_format(date_create($pensioner->effect_of_pension), "d-m-Y").'</b>.<br/>&nbsp';
			...
			<tr>
				<td colspan="4" style="border:1px solid#000">
					 1.W.e.f <?php echo nbs(2).''.$year80;?> to <?php  echo nbs(2).''. $year85; ?>(80 yrs.) 20% increase on  Rs. <?php echo $pensioner->getEnhanceRate_sanjay()?>= <?php echo  round((20*$pensioner->getEnhanceRate_sanjay())/100)+$pensioner->getEnhanceRate_sanjay()?>  <br />
					...
					?> to <?php echo nbs(2).''. $year90; ?>(85 yrs.) 30% increase on  Rs. <?php echo $pensioner->getEnhanceRate_sanjay();?>=<?php echo round((30*$pensioner->getEnhanceRate_sanjay())/100)+$pensioner->getEnhanceRate_sanjay()?><br />
					...
					?> to <?php echo nbs(2).''. $year95; ?>(90 yrs.) 40% increase on  Rs. <?php echo $pensioner->getEnhanceRate_sanjay();?>= <?php echo round((40*$pensioner->getEnhanceRate_sanjay())/100)+$pensioner->getEnhanceRate_sanjay()?><br />
					...
					?> to <?php echo nbs(2).''. $year100;?>(95 yrs.) 50% increase on  Rs. <?php echo $pensioner->getEnhanceRate_sanjay();?>= <?php echo round((50*$pensioner->getEnhanceRate_sanjay())/100)+$pensioner->getEnhanceRate_sanjay()?><br />
                    5.W.e.f <?php $year100plus = new DateTime($year100);
					$year100plus->modify('+1 day');
					$year100plus1 = date_format($year100plus,"Y-m-d");
					echo nbs(2).''. $year100plus1;?> to (100 yrs.) onwards 100% increase on  Rs. <?php echo $pensioner->getEnhanceRate_sanjay();?>= <?php echo round((100*$pensioner->getEnhanceRate_sanjay())/100)+$pensioner->getEnhanceRate_sanjay()?><br />
				</td>	
			</tr>
