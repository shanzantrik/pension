...
/**/
	...
	if($NumberOfWives=="3"){
		...
		{
			//$enhance_from_upto="<b>E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$enhan_upto->format('d-m-Y')."</b>";
			$enhance_from_upto="<b>11E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to 31-12-2015</b>";
		}
		elseif($age<25)
		{
			$enhance_from_upto="<b>22E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$pensioner->getPensionEndDate()." thereafter no family pension</b>";
		}
		elseif($age>=25)
		{
			//$enhance_from_upto="<b>E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$enhan_upto_seven->format('d-m-Y')."</b>";
			$enhance_from_upto="<b>33E/R of Rs.".$return."/- PM".$pensioner->getDRMA()." w.e.f. ".$enhan_from->format('d-m-Y')." to ".$pensioner->getPensionEndDate()." thereafter no family pension</b>";
		}
	}
	...
	//seven pay
	if($NumberOfWives=="3" && $pensioner->pay_commission==7){
		$return = round($x_amount*50/100)/$NumberOfWives;
	...
	}else if($NumberOfWives=="4" && $pensioner->pay_commission==7){
...
/*else if($pensioner->dor!="0000-00-00" && $pensioner->dod!="0000-00-00"){
    ...
    $pensioner->salutation=$pensioner->salutation;
    $enhance_from_upto="<b>from ".$enhan_from->format('d-m-Y')." upto ".$enhan_upto->format('d-m-Y')."</b>";
	$ordinary_from=$enhan_upto->modify('+1 day');
	$ordinary_from_upto="<b>from ".$ordinary_from->format('d-m-Y')." until his/her remarriage or death"."</b>";
	...
	<div id="print1<?php echo $i+1 ?>" style="width: 1000px; margin: 0px auto;">
				...
				<tr>
			 	...
			 	<tr>
					<td style="vertical-align: top;" colspan="4" align="left"><?php echo nbs(13); ?>The pension payment orders/GPO whose details are given below in favour of &nbsp;<b><?php 
						...
						if($NumberOfWives==1){
							if($pensioner->dor==$pensioner->dod){
								echo $pensioner->getNameOfLegalHeir().$sentence_of_lifetime_arear;	
							}else{
						...
						}?> </b>&nbsp;are forwarded herewith for payment from your end.</td>
				</tr>	
			 	<tr>
		...
		3.  (a) Adhoc/Temporary increase
		    (b) Amount of basic pension 				:-<b>Rs.<?php if($pensioner->pay_commission==7){ echo  ($total_amount*50/100)/$NumberOfWives;} elseif ($pensioner->pay_commission==7 && $class_of_pension=="Parents_Pension") {
		    	echo "N/A";
		    } else{echo $amount_pension/$NumberOfWives;} echo "/- PM+DR+MA";?> 
		    	
		    </b>
		    (c) Relief on pension 						:- As per PPO
		    (d) Family Pension in case of death of the Pensioner 	:-<b>Rs.<?php if($pensioner->pay_commission==7){ echo  ($total_amount*50/100)/$NumberOfWives;} else{echo $amount_pension;}?>/- PM+DR+MA</b> 
			  ...
			  */
						...
						if($age>=2018 || $age==0)
						{
							if($NumberOfWives>1)
							{
								...
								$output=$pensioner->getEnhanceRate();
								$final_output=explode(" ", $output);

								if(!empty($final_output)){
									echo $final_output[0]." /- PM+DR+MA";
								}else{
									echo "NA/-";
								} 
							}
...
*/
				...
				<tr>
					<td style="vertical-align: top;" colspan="4">
						...
						?> ,<br /><?php echo $pensioner->getAddressAfterRetirement($i); ?></b><br /><br />
	...
	<div id="print2<?php echo $i+1 ?>" style="width: 1000px; margin: 0px auto;">
		...
		<div style="width:1000px; min-height:600px; font-size: 1.2em; color:#000000; background-color:#FFFFFF; line-height: 1.5em">
			...
			<table width="100%" border="0" cellpadding="2" id="report">
				...
				<tr>
					<td colspan="4"><div align="left" style="text-align: justify;"><?php 
					echo nbs(6); ?> I am to request that you will be so good as to arrange for payment a sum of <b>Rs. <?php //if($pensioner->pay_commission==7){ echo ceil($bincs);} else{
					...
					?> </b> being the amount of retirement/death gratuity sanctioned to him/her in letter no. <b><?php echo $gpo_no; ?></b> from the Director of Audit & Pension, debitable to 2071 Pension & ORB etc.</div></td>
				</tr>
				<tr>
				...
				<?php
					...
					<td colspan="2" valign="top" style="padding: 10px 0;">
		                ...
		                if($pensioner->com_applied == 1){
			                if($pensioner->pay_commission==7 && $pensioner->dor==$pensioner->dod){ 
			                		echo 'N/A';
			                }elseif($pensioner->pay_commission==7){
			                	echo $bigbazars;
			                } else{
				...
				<tr>
		      	...
		      	<tr>
					...
					//if($pensioner->pay_commission==7){ echo $total_amount*50/100;} else{echo $pensioner->getEnhanceRate_sanjay();}
						...
						{
							if($NumberOfWives==1){
								$t_output=$pensioner->getEnhanceRate(); 
								$temp_final_output=explode(" ", $t_output);

								if(!empty($temp_final_output)){
									echo $temp_final_output[0]."/- PM+DR+MA";
								}else{
									echo "NA";
								}	
							}else{
			...
			<tr>
		 	...
		 	<tr>
				<td style="vertical-align: top;" colspan="2">
					...
					<div style="float: left;"><b><!--<?php echo nbs(2).''. $spouse.'('.$spouse_type.') ';?>of<?php echo nbs(1).strtoupper($pensioner->salutation." ".$pensioner->name); ?>, <br /><?php echo nbs(1); ?>Ex. <?php echo $pensioner->designation; ?>-->
					<?php 
			...
			<tr>
				<td style="vertical-align: top;" colspan="4">
					<table border="1" cellspacing="2" cellpadding="2" id="inner-table">
						<tr>
							<th width="20%" style="vertical-align: top;font-size: 12px;">Class of Pension and date of commencement</th>
							<th width="0%" style="vertical-align: top;font-size: 12px;">Date of Birth</th>
							<th width="0%" style="vertical-align: top;font-size: 12px;">Religion and Nationality</th>
							<th width="30%" style="vertical-align: top;font-size: 12px;">Residence showing village pargana</th>
							<th width="20%" style="vertical-align: top;font-size: 12px;">Amount of monthly pension</th>
						</tr>
						<tr>
							<td style="vertical-align: top; text-align: center;font-size: 12px;" width="20%"><?php echo str_replace("_", " ", $class_of_pension); ?><br/>w.e.f <?php echo $pensioner->dateTimeToDate($pensioner->effect_of_pension); ?></td>
							<td style="vertical-align: top; text-align: center;font-size: 12px;" width="0%"><?php echo $pensioner->getDOBOfLegalHeirX($i+1); ?></td>
							<td style="vertical-align: top; text-align: center;font-size: 12px;" width="0%"><?php echo $pensioner->religion.", ".$pensioner->nationality;?></td>
							<td style="vertical-align: top; text-align: center;font-size: 12px;" width="30%"><?php echo $pensioner->getAddressAfterRetirement($i); ?></td>
							<td style="vertical-align: top; text-align: center;font-size: 12px;" width="20%"><!--Life time arrear pension of Rs. -->
							<?php 
...
echo nbs(6);?> UNTIL FURTHER NOTICE and on the expiration of every month, be pleased to pay to <b> <?php 
/*if($NumberOfWives==1){
						echo $pensioner->getNameOfLegalHeir();	
					}else{
						echo $pensioner->getNameOfLegalHeirX($i+1);
					...
					?> <?php echo '</b>, a sum of <b>Rs.'.$orrate/$NumberOfWives.'/-</b> PM+DR+MA only (less income-tax), being the amount of <b>'.strtoupper(str_replace("_", " ", $class_of_pension)).'</b>, as Arunachal Pradesh Government Pensioner upon the production of this Order and a receipt according to usual form.<br/>
The payment should commence from '.date_format(date_create($pensioner->effect_of_pension), "d-m-Y").'</b>.<br/>&nbsp';//strtoupper($spouse).'('.strtoupper($spouse_type).')'.' of Late '.$pensioner->name.' Ex-'.$pensioner->designation
			...
			<tr>
		 	...
		 	<tr>
				<td style="vertical-align: top;font-size: 12px;" colspan="2"><b>
					To<br/>
					<!-- Minor Head<br/>
