	...
	<div id="print1" style="width: 900px; margin: 0px auto;">
		...
		<table width="100%" border="0" cellpadding="2" id="report">
			...
			<tr>
				...
				<td colspan="5">
					<!-- (i) Rs <?php if(!$val['total_cal_sav_amt_group_d']=="0"){ echo round($val['total_cal_sav_amt_group_d']); } else { echo ""; } if(!$val['total_cal_sav_amt_group_c']=="0"){ echo round($val['total_cal_sav_amt_group_c']); } else { echo ""; }?><?php if(!$val['total_cal_sav_amt_group_c']=="0"){ echo round($val['total_cal_sav_amt_group_c']); } else { echo ""; }?><?php if(!$val['total_cal_sav_amt_group_a']=="0"){ echo round($val['total_cal_sav_amt_group_a']); } else { echo ""; }?> (Rupees ______) only under Sub-head-Saving Fund Under Major head of Account-8011 Insurance and Pension Fund of Union Territory Government Employees Group Insurance Scheme. -->
					(i) Rs <b><?php echo $total; ?> /- (Rupees <?php echo no_to_words($total); ?>)</b> only under Sub-head-Saving Fund Under Major head of Account-8011 Insurance and Pension Fund of Union Territory Government Employees Group Insurance Scheme.
				</td>
			</tr>
			<tr>
				<td colspan="5">(ii) Rs <b><?php if(!$final_insurance_amt=="0"){ echo " ".$final_insurance_amt."  "."(Rupess".no_to_words($final_insurance_amt).")";} else{echo "----------------";}?></b> only under Sub-head-Insurance Fund Under Major head of Account-8011 Insurance and Pension Fund of Union Territory Government Employees Group Insurance Scheme.</td>
			</tr>  
			<tr>
