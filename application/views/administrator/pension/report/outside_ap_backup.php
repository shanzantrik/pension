...
<div id="form1" style="display: block;">
	...
	<div id="print1" style="width: 1000px; margin: 0px auto;">
		<div style="width:1000px; min-height:600px; font-size: 1.2em; color:#000000; background-color:#FFFFFF; line-height: 1.5em">
			...
			<table width="100%" cellpadding="3" id="report" border="0">
				...
				<tr>
			 	...
			 	<tr>
					<td style="vertical-align: top;" colspan="4" align="left"><?php echo nbs(13); ?>The pension payment orders/GPO/CPO whose details are given below in favour of &nbsp;<b><?php echo strtoupper($name); ?>,  <?php echo $pensioner->designation;?>,<?php echo $pensioner->sub_designation;  ?></b>&nbsp;are forwarded herewith for payment from your end.</td>
				</tr>	
			 	<tr>
		...
		3. (a) Adhoc/Temporary increase
		    (b) Amount of basic pension 				:- <b><?php 
		    ...
		    {?> Rs. <?php echo $pensioner->getAmountofPension(); ?> /- PM <?php }

		    ?> <?php echo $pensioner->getDRMA(); ?></b>
		    (c) Relief on pension 						:- As per PPO
		    (d) Family Pension in case of death of the Pensioner 	:- <b><?php 
			if($pensioner->pay_commission==7)
			{?> Rs. <?php echo  $total_amount*50/100;?>/- PM <?php } 
		    elseif($pensioner->class_of_pension=='Extraordinary_Pension' && $pensioner->pay_commission==6 && $pensioner->getAmountofPension() < 7000)
			{echo 'Rs. 7000/- PM';} 
			else
			{?> Rs. <?php echo $pensioner->getAmountofPension();?>/- PM <?php } 
		    ?> <?php echo $pensioner->getDRMA(); ?></b>
			  i) Enhanced rate 						:- <b><?php 
              
              
	            if($pensioner->pay_commission==7 && $class_of_pension =="Parents_Pension")
			    { echo "N/A";} 
			    elseif($pensioner->pay_commission==7)
              	{?> Rs. <?php echo ($total_amount*50/100);
              	} 
				elseif($pensioner->class_of_pension=='Extraordinary_Pension' && $pensioner->pay_commission==6 && $pensioner->getAmountofPension() < 7000)
			    {echo 'N/A';} 
                else 
              	{?> Rs. <?php echo $pensioner->getEnhanceRate();
			  	}?></b>
						</pre>
					</td>
				</tr>
				...
				<tr>	
					<td style="vertical-align: top;" colspan="4" style="text-align:left;"><?php echo nbs(11); ?>ii) Ordinary rate :- <b><?php 
					if($pensioner->pay_commission==7)
					{?> Rs. <?php echo ($total_amount*50/100-$total_amount*50/100*40/100);?>/- PM <?php }
				    elseif($pensioner->class_of_pension=='Extraordinary_Pension' && $pensioner->pay_commission==6 && $pensioner->getAmountofPension() < 7000)
			        {echo 'N/A';} 
					else 
					{?> Rs. <?php echo $pensioner->getOrdinaryRate(); ?>/- PM <?php } 
				    ?> <?php echo $pensioner->getDRMA(); ?></b>  (Until his/her remarriage or death)</td>
				...
				<?php
				$bina1=substr($pensioner->total_service,0,2);

				//echo $bina;
				$binb1=$pensioner->total_amount;
				$biny1=substr($pensioner->total_service,9,1);
				//echo $biny;
				$fourdaamount=($binb1)+($binb1*4/100);
				$anotheramount=$fourdaamount*(0.25)*($pensioner->smp);
				
				if($bina1 > 33 && $pensioner->dor < '2017-06-30')
				{
				 $binc1=$anotheramount;
				 //$binc1=1;
					
				}
				else if($bina1 < 33 && $biny1 < 9  && $pensioner->dor > '2016-06-31')
				{
					//$binc=($binb)*(1/4)*($bina*2)+1;
					//$dacal=($binb)*(2/100);
					
					$binc1=($binb1*2/100+$binb1)*($pensioner->smp)*(1/2);
					//$binc1=2;
					
					//$binch=($binb*2/100+$binb)*(1/2)*($bina*2);
					
				}
				else if($pensioner->dor < '2016-06-31')
				{
					//$binc1=($binb)*(1/4)*($bina*2);
					$binc1=($binb1)*(1/4)*($pensioner->smp);
					//$binc1=3;
				}
				else
				{
					$binc1=($binb1)*(1/4)*($bina1*2);
					//$binc1=4;
				}
				?>
				<tr>
				...
				{
					$vivo=$row['comm_value'];
					$bigbazar=ceil(($total_amount)*50/100*0.4*$vivo*12);
					$visal=($total_amount)*50/100*40/100;
					$sohum=($total_amount)*50/100;
				?>
				<tr>
					<td style="vertical-align: top;" colspan="4">
					5. No.<?php echo $cpo_no; ?> and amount of <b>Rs. <?php 
					if($pensioner->pay_commission==7)
					{ echo $bigbazar;} 
				    else
			...
			?>
				...
				<tr>
					<td style="vertical-align: top;" colspan="4">
						...
						1) <?php echo strtoupper($name).',<br />'.nbs(5).str_replace(",", ",<br />".nbs(4), $pensioner->address_after_retirement).$pin; ?><br /><br />
						2) <?php echo str_replace(",", ",<br />".nbs(4), $pensioner->office_address); ?><br/>
	...
	<div id="print3" style="width: 1000px; margin: 0px auto;">
		<div style="width:1000px; min-height:600px; padding: 0 20px; font-size: 1.3em; color:#000000; background-color:#FFFFFF; line-height: 2em">
			...
			<table width="100%" border="0" cellpadding="2" id="report">
				...
				<tr>
					<td colspan="3"></td>
					<td style="">Director/Joint Director</td>
					<td></td>
				</tr>
				<tr>
	...
	<div id="print77" style="width: 1000px; margin: 0px auto;">
		<div style="width:1000px; min-height:600px; padding: 0 20px; font-size: 1.1em; color:#000000; background-color:#FFFFFF; line-height: 2em">
			...
			<table width="100%"  cellpadding="2" id="report">
				...
				<p>
					It is to state that the P.P.O. No.<strong><?php echo $pensioner->ppo_no;?></strong> issued in favour of <strong><?php echo strtoupper($name); ?>,&nbsp;<?php echo $pensioner->designation; ?>,&nbsp;<?php echo $pensioner->sub_designation; ?></strong>  Authorizing Superannuation Pension/Retiring pension w.e.f. <strong><?php echo $date;?></strong> was forwarded to you vide this office letter No. <strong><?php echo $pensioner->case_file_no; ?></strong>  <strong>dated  04-07-2016.</strong> Consequent on revision of pay in terms of OM No. DAP/PEN/27/2016, dated 04-07-2016 (7th CPC), the pensionery benefit in respect of <strong><?php echo strtoupper($name); ?>,<?php echo $pensioner->designation; ?> </strong> is now worked out as under:-
				</p>
				<br/>
				<p>i)	Revised Superannuation/ Retiring pension at the rate of <strong>Rs.<?php echo $total_amount*50/100;  ?>/- (Rupees <?php echo no_to_words($total_amount*50/100); ?> )</strong> only P.M. plus D.R. & MA w.e.f. <strong>01-07-2016</strong> after adjusting the Pension, ,DR and MA already paid vide P.P.O. No.<strong><?php echo $pensioner->ppo_no;?></strong></p>
				<br/>
				<p>
					ii)	In the event of death of the pensioner, revised family pension at enhance rate of <strong>Rs.<?php echo $total_amount*50/100;  ?>/- (Rupees <?php echo no_to_words($total_amount*50/100); ?> )</strong> p. m. plus DR & MA for a period of 7 years or till the pensioner would have attained the age of 67 yrs, had he been survived whichever is earlier, and thereafter family pension at the rate of <strong>Rs.<?php echo ($total_amount*50/100-$total_amount*50/100*40/100); ?>/- (Rupees <?php echo no_to_words($total_amount*50/100-$total_amount*50/100*40/100); ?> )</strong> only at normal rate is admissible to <strong><?php echo $pensioner->getNameofSpouse(); ?>whose date of birth is <?php echo $pensioner->getDOBofSpouse(); ?></strong>
				</p>
				<br/>
				<!--<p>
					iii)	Revised gratuity Rs.<?php //echo $diff; ?>/- (Rupees ) only is sanctioned now. The gratuity already drawn is Rs.<?php //echo $pensioner->sixgratu; ?>/- (Rupees ) only under GPO No. Pen/AP/GPO/. Hence net amount of Rs.<?php //echo $diff;?>/- (Rupees ) only may be paid now.
				</p>-->
				<p>
					iv)	Revised commutation of pension of <strong> Rs.<?php if($pensioner->pay_commission==7 && $pensioner->com_applied==0){ echo "NA"; } else if($pensioner->pay_commission==7){ echo $binc;} else{ echo $pensioner->getDCRG(); }?>/- (Rupees )</strong> only being revised commuted value of <strong>Rs./-<?php if($pensioner->pay_commission==7 && $pensioner->com_applied==0){ echo "NA"; } else if($pensioner->pay_commission==7){ echo $binc;} else{ echo $pensioner->getDCRG(); }?> (Rupees )</strong> only out of his revised pension of <strong>Rs./-<?php if($pensioner->pay_commission==7 && $pensioner->com_applied==0){ echo "NA"; } else if($pensioner->pay_commission==7){ echo $binc;} else{ echo $pensioner->getDCRG(); }?> (Rupees )</strong> only is sanctioned now. The commuted value of pension of <strong> Rs./- <?php if($pensioner->pay_commission==7 && $pensioner->com_applied==0){ echo "NA"; } else if($pensioner->pay_commission==7){ echo $binc;} else{ echo $pensioner->getDCRG(); }?>(Rupees  )</strong> only already authorization earlier vide No. <strong>AIS/COM/026</strong> and hence difference of commutation of pension of <strong>Rs./-<?php if($pensioner->pay_commission==7 && $pensioner->com_applied==0){ echo "NA"; } else if($pensioner->pay_commission==7){ echo $binc;} else{ echo $pensioner->getDCRG(); }?> (Rupees )</strong> only is now authorized for payment. Consequent on revised commutation, the reduced pension of <strong>Rs.<?php  echo ($total_amount*50/100-$pensioner->getAmountofPension()*40/100);   ?>(Rupees&nbsp;<?php echo no_to_words($total_amount*50/100-$pensioner->getAmountofPension()*40/100); ?> )</strong> only p.m. plus D.R. & M.A. may be paid to <strong><?php echo strtoupper($name); ?>,<?php echo $pensioner->designation; ?></strong>
				</p>
				<br/>
				<p>
					In view of the above, I am to request you to advise the <strong><?php echo $pensioner->bank_name;?></strong> for making payment of revised pension to <strong> <?php echo strtoupper($name); ?>,&nbsp;<?php echo $pensioner->designation; ?></strong>  after keeping a note to this effect in both halves of P.P.O. No. <strong><?php echo $pensioner->ppo_no;?></strong> under proper attestation and after adjusting all the payments already made.
				</p>
				<br/>
				<p>
					The amount is debatable to the Head of Accounts “2071”- pension & ORB.,Central Govt. "o4" ordinary pension(AIS)<br/>
				...
				<table border="1" width="90%">
					<tr>
					    <th>Age of pensioner/Family pensioner</th>
					    <th>Additional quantum of pension</th>
				...
				<div style="margin-left:439px;float:left;">
				Dated Naharlagun, the ………………
				</div>
				<br/><br/><br/><br/><br/><br/>
				<table>
					<tr>
				...
				<div style="margin-left:390px;">Joint Director of Audit & Pension<br/>
                    Directorate of Audit & Pension<br/>
                    Naharlagun.</div>
		</div>
	</div>
					...
					<tr>
						<td style="vertical-align: top;" colspan="3">
							<div id="heading" style="text-align:center; padding:10px; font:Arial, Helvetica, sans-serif; font-size:16px">
						        <div style="font-family: initial; margin-left: 200px;font-weight:bold;margin-top:-22px;">FORM-43<br/>(See paragraph 306)<br/>PENSION PAYMENT ORDER<br/>DISBURSER'S PORTION</div>
						    </div>
						</td>
		    ...
		    ?><br/>w.e.f <?php echo $pensioner->dateTimeToDate($pensioner->effect_of_pension); ?></b></td>
					...
					<tr>
						<td style="vertical-align: top; text-align:center;" colspan="4">
							<div style="font-size: 15px; margin-top: 5px;font-weight:bold;">OFFICE OF THE DIRECTOR OF AUDIT & PENSION<br/>GOVERNMENT OF ARUNACHAL PRADESH, NAHARLAGUN</div>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td style="vertical-align: top; text-align: justify;" colspan="4">
							...
							<?php
									...
									echo '<br/>2.In the event of the death of <b>'.strtoupper($name).'</b>, Family Pension of <b>Rs. '.$pensioner->getAmountofPension().'/- PM'.$pensioner->getDRMA().'</b>, may be paid to <b>'.$pensioner->getNameofSpouse().'</b>, whose date of birth is <b>'.$pensioner->getDOBofSpouse().' </b> from the day following the date of death of <b>'.strtoupper($name).'</b> for a period of 7 years, or for a period up to the date on which '.$pensioner->pensioner_pronoun.' would have attained the age of '.$pensioner->pension_attained_age.'/67 years, had '.$pensioner->pensioner_pronoun.' survived, whichever is less; thereafter normal rate of Family pension of <b>Rs. '.$pensioner->getOrdinaryRate().'/- PM'.$pensioner->getDRMA().'</b>, till the date of her/his remarriage or death whichever is earlier (on receipt of death certificate and Form of application from widow/widower). Further, the quantum of pension/Family pension shall be increased as follows:-<br /><br />';	
					...
					<tr>
						...
						<b>Quantum of Additional Pension/Family Pension</b><br/>
							1.Age W.e.f <?php echo $pensioner->dateTimeToDate($year80);?> to <?php echo $pensioner->dateTimeToDate($year85); ?>(80 yrs.) 20% increase on  Rs. <?php echo $pensioner->getAmountofPension(); ?>= <?php  echo round((20*$pensioner->getAmountofPension())/100)+$pensioner->getAmountofPension();?>  <br />
							2.Age W.e.f <?php $year85plus = new DateTime($year85);
							...
							3.Age W.e.f  <?php $year90plus = new DateTime($year90);
							...
							4.Age W.e.f <?php $year95plus = new DateTime($year95);
							...
							5.Age W.e.f <?php $year100plus = new DateTime($year100);
	...
	<div id="disburser_pensioner_portion" style="display: none;">
		...
		<div id="print7" style="width: 1000px; margin: 0px auto;">
			<div style="width:1000px; min-height:600px; padding: 0 10px; font-size: 1 em; color:#000000; background-color:#FFFFFF; line-height: 1.5em">
				<table width="95%" cellpadding="3" id="report" border="0">
					...
					<tr>
						<td style="vertical-align: top;" colspan="3">
							<div id="heading" style="text-align:center; padding:10px; font:Arial, Helvetica, sans-serif; font-size:16px">
						        <div style="font-family: initial; margin-left: 200px;margin-top:-22px;">FORM-43<br/>(See paragraph 306)<br/>PENSION PAYMENT ORDER<br/>PENSIONER'S PORTION</div>
						    </div>
						</td>
						<td style="vertical-align: top;" align="right" rowspan="3"><div id="photograph" style="width: 150px; height: 128px; border: 1px solid #bababa; border-radius: 3px; -moz-border-radius: 3px; margin-top: 63px;"><p style="text-align: center; margin-top: 50px; font-weight: bold;">Photograph</p></div></td>
					</tr>
		    ...
		    ?></b></td>
					...
					<tr>
						<td style="vertical-align: top; text-align:center" colspan="4">
							<div style="font-size: 15px; margin-top: 10px;font-weight:bold;">OFFICE OF THE DIRECTOR OF AUDIT & PENSION<br/>GOVERNMENT OF ARUNACHAL PRADESH, NAHARLAGUN</div>
						</td>
					</tr>
					<tr>
						<td style="vertical-align: top;" colspan="2"><strong>No. <?php echo $pensioner->case_file_no."/".$pensioner->ppo_no; ?></strong></td>
					...
					<tr>
						<td style="vertical-align: top;" colspan="2">
							...
							c) Excess Gratuity Paid : <?php echo $pensioner->provisional_gratuity - $pensioner->getDCRG(); ?> 
