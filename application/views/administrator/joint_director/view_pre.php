<script type="text/javascript">
	...
	/*  function ajax1(x,y){
   //alert("okk");
   		$.get('<?php echo site_url('administrator/joint_director/jd_confirm_from_ips');?>', {file: x}, function(data){
		alert(data);
			if(data=='ok'){
				$("#"+y).addClass("btn btn-success");
				$("#"+y).attr("disabled","true");
    ...
    <tbody>
        ...
        <?php $i=1256; ?>
            <?php foreach ($lists as $key):?>
                ...
                <td><a title="Attach Checklist for this claimant from receipt branch" href="<?php echo site_url('/administrator/joint_director/view_checklist/'.base64_encode($key->file_no))?>/Receipt" class="open-dialog-edit btn btn-success btn-rad" data-id=""><i class="icon-book"></i>Checklist</a>
<a title="Attach IPS for this claimant from receipt branch" href="<?php echo site_url('/administrator/joint_director/attach_authority/'.base64_encode($key->file_no))?>/Pension" class="open-dialog-edit btn btn-success btn-rad" data-id=""><i class="icon-book"></i>Authority</a>
                </td>
                </tr>
            <?php endforeach ?>
    </tbody>
...
<br/>
<a href="#myDel1" data-toggle="modal" class="btn btn-primary">Forward</a>
