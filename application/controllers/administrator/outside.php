...
class outside extends CI_Controller {
	...
	function superannuation_pension($serial_no)
	{
		...
		if($cop!='Superannuation_Pension') {
			...
			$data['content'] = $this->load->view('administrator/pension/report/outside_ap', $vrp, true);
	...
	function normal_family_pension($serial_no)
	{
		...
		if($cop!='Normal_Family_Pension') {
			...
			if($dor_y[0]>2006){
				...
				$data['content'] = $this->load->view('administrator/pension/report/family/outside/normal_family_pension_06_outside', $vrp, true);
				$this->load->view('administrator/default_template', $data);
			}elseif($dor_y[0]<2006){
	           	//5th pay commission
	            $vrp['account_enfacement'] ="true";
				$data['title'] = "Normal Family Pension";
				$data['content'] = $this->load->view('administrator/pension/report/family/outside/normal_family_pension_05_outside', $vrp, true);
				$this->load->view('administrator/default_template', $data);
			}
