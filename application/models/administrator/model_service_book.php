...
class Model_service_book extends CI_Model {
	...
	function getPPONo() {
		...
		$this->db->where('designation not like', 'PCCF& Prl.Secy. (E&F)%');
		$result = $this->db->get('pensioner_personal_details');
	    $row = $result->result();
	    if($row[0]->ppo_no == '') {
	    	return "23098";//22533
	...
	function getGPONo() {
		...
		$result = $this->db->get('pensioner_personal_details');
	    $row = $result->result();
	    //$temp_id=explode("/", $row[0]->gpo_no);
	    //echo print_r($row[0]->gpo_no);
	    //die();
	    if($row[0]->gpo_no == '') {
	    	return "30131";//29566
	    } else {
...
$this->db->query("INSERT INTO increment_detail (case_no, from_date,to_date,rate_of_pay,amount) VALUES ('$case','$fidate[$i]','$toidate[$i]','$irate[$i]','$iamount[$i]')");
	...
	function update() {
		...
		if($class_of_pension=='Normal_Family_Pension'){//  && $wife_count>1){
			...
			if(is_array($_POST['address_after_retirement'])){
				$address_after=array();
...
$this->db->query("INSERT INTO increment_detail (case_no, from_date,to_date,rate_of_pay,amount) VALUES ('$case','$fidate[$i]','$toidate[$i]','$irate[$i]','$iamount[$i]')");
	...
	function reportIO($serial_no) {
		...
		if($query->num_rows()>0) {
			$row = $query->row(); 
			$check_tg=explode('s:', $row->sub_to);
			if(!empty($check_tg[1])){
				$treasury_officer = unserialize($row->sub_to);
				//print_r($treasury_officer[0]['sub_to']);

				if($treasury_officer[0]['sub_to']!=""){ //$row->sub_to!="") {
					return "exists";
				} else {
					return "not_exists";
				}
