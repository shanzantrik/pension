...
class Model_service_book extends CI_Model {
	...
	function getGPONo() {
		...
		$this->db->where('designation not like', 'PCCF& Prl.Secy. (E&F)%');//Designation Filter 13-02-2020
		$result = $this->db->get('pensioner_personal_details');
	    $row = $result->result();
	    if($row[0]->gpo_no == '') {
	    	return "30131";//29566
	    ...
	    $result = $this->db->query("SELECT max(`gpo_no`) as 'gpo_no'  FROM `pensioner_personal_details` WHERE `designation` NOT LIKE 'PCCF& Prl.Secy. (E&F)%' AND `designation` NOT LIKE 'IAS%' AND `designation` NOT LIKE 'IPS%' AND `designation` NOT LIKE 'IFS%'");
	    $row = $result->result();
	    if($row[0]->gpo_no == '') {
	    	return "30131";//29566
	    } else {
	...
	function getCPONo() {
		...
		$this->db->where('designation not like', 'IFS%');
		$result = $this->db->get('pensioner_personal_details');*/
		$result = $this->db->query("SELECT max(Convert(`cpo_no`, SIGNED)) as 'cpo_no'  FROM `pensioner_personal_details` WHERE `designation` NOT LIKE 'PCCF& Prl.Secy. (E&F)%' AND `designation` NOT LIKE 'IAS%' AND `designation` NOT LIKE 'IPS%' AND `designation` NOT LIKE 'IFS%'");
	    $row = $result->result();
	    if($row[0]->cpo_no == '') {
	...
	public function checkDuplicate($table,$column,$data) {
	...
	function add() {
        ...
        $ppo_ais			= $this->security->xss_clean($this->input->post('ppo_file_no_ais'));
			...
			if($this->checkDuplicate("pensioner_personal_details","ppo_no",$ppo_no)){
				$ppo_no = get_increamented_option_value('ppo_number');
			}
			...
			//gpo
			if($this->checkDuplicate("pensioner_personal_details","gpo_no",$gpo_no)){
				$gpo_no = get_increamented_option_value('gpo_number');
			}
			...
			//cpo
			if($this->checkDuplicate("pensioner_personal_details","cpo_no",$cpo_no)){
				$cpo_no = get_increamented_option_value('cpo_number');
			}
		...
		$lap_from 		= $this->security->xss_clean($this->input->post('lapfrom'));
		$lap_to 		= $this->security->xss_clean($this->input->post('lapto'));
		$lap_amount 		= $this->security->xss_clean($this->input->post('lapamount'));
		...
		$pensioner_personal_details = array('serial_no'=>$serial_no, 'cash_received'=>$cash_received, 'class_of_pension'=>$class_of_pension, 'case_no'=>$case_no, 'ppo_no'=>$ppo_no, 'gpo_no'=>$gpo_no, 'cpo_no'=>$cpo_no, 'salutation'=>$salutation, 'name'=>$name, 'dob'=>$dob, 'religion'=>$religion, 'nationality'=>$nationality, 'category'=>$category, 'sex'=>$sex, 'designation'=>$designation, 'department'=>$department, 'blood_group'=>$blood_group, 'idcard_serial_no'=>$idcard_serial_no,
			'lap_from'=>$lap_from,
			'lap_to'=>$lap_to,
			'lap_amount'=>$lap_amount,'ppo_ais'=>$ppo_ais
	);
...
$this->db->query("INSERT INTO increment_detail (case_no, from_date,to_date,rate_of_pay,amount) VALUES ('$case','$fidate[$i]','$toidate[$i]','$irate[$i]','$iamount[$i]')");
	...
	function update() {
		...
		$lap_amount 		= $this->security->xss_clean($this->input->post('lapamount'));
		$ppo_ais			= $this->security->xss_clean($this->input->post('ppo_file_no_ais'));
		$name_ais= $this->security->xss_clean($this->input->post('ais_report_name'));
		$designation_ais= $this->security->xss_clean($this->input->post('ais_report_designation'));
		$department_ais= $this->security->xss_clean($this->input->post('ais_report_servicebelong'));
		$batch_ais= $this->security->xss_clean($this->input->post('ais_report_batch'));

		if($lap_from==NULL or $lap_from==''){$lap_from='0000-00-00';}
		if($lap_to==NULL or $lap_to==''){$lap_to='0000-00-00';}
	...
	'lap_from'=>$lap_from,
			...
			'lap_amount'=>$lap_amount,'ppo_ais'=>$ppo_ais,
			'name_ais'=>$name_ais,
			'designation_ais'=>$designation_ais,
			'department_ais'=>$department_ais,
			'batch_ais'=>$batch_ais);
