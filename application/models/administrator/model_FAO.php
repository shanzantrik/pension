...
class Model_FAO extends CI_Model
{
	...
	function getAll_file_gissuperintendent()
	 ...
	 {
	    $q=$this->db->query("SELECT * from file_status a,pension_receipt_file_master b,checklist c,token_reciept d, pension_receipt_register_master e where b.file_No=d.file_No and a.file_no=b.file_No and b.file_No=c.file_no and a.status='gis_Forwarded to FAO' and b.dept_forw_no=e.dept_forw_no and e.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	 ...
	 {
	    $q=$this->db->query("SELECT * from file_status a,pension_receipt_file_master b,token_reciept c, pension_receipt_register_master d where b.file_No=c.file_No and  a.file_no=b.file_No and a.status='ips_Forwarded to FAO' and b.dept_forw_no=d.dept_forw_no and d.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	 ...
	 {
	    $q=$this->db->query("SELECT * from file_status a,pension_receipt_file_master b,token_reciept c,pensioner_personal_details d, pension_receipt_register_master e where d.case_no=b.file_No and c.file_No=b.file_No and a.file_no=b.file_No and a.status='pension_Forwarded to FAO' and b.dept_forw_no=e.dept_forw_no and e.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	...
	function pension_save_fwd_by_fao()
	{
	 $allocated_date=date('Y-m-d');
		...
		elseif(!empty($_POST['send_back'])){
			...
			if($send_back=='pension_superintendent')
			{
					$chk=@$_POST['chk_receipt'];
					if(empty($chk)){
					return 'validate';
					...
					foreach($chk as $a => $b)
					{
					   	$file_tracking=array(
								'file_no'=>$chk[$a],
								'branch'=>'Pension', //1004 means director
								'file_status'=>'Send back to Pension Superintendent',
								'member_code'=>$this->session->userdata('member_code')
							);
