...
class Model_Gis extends CI_Model
{
  ...
  //exit();
	...
	function getAll()
	 ...
	 {
	    $q=$this->db->query("SELECT * from file_status a,pension_receipt_file_master b,token_reciept c, pension_receipt_register_master d where a.file_no=b.file_No and b.file_No=c.file_No and a.status='Forwarded to GIS From Receipt' and b.dept_forw_no=d.dept_forw_no and d.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	 ...
	 {
	    $q=$this->db->query("SELECT * from file_status a,pension_receipt_file_master b,checklist c,token_reciept d, pension_receipt_register_master e where b.file_No=d.file_No and a.file_no=b.file_No and b.file_No=c.file_no and a.status='gis_Forwarded to GIS DA' and b.dept_forw_no=e.dept_forw_no and e.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	  function getFileFromPensionDA()
	 {
	 ...
	 {
		$q=$this->db->query("SELECT * from file_tracking_details a,pension_receipt_file_master b,file_status c, pension_receipt_register_master d where a.file_no=b.file_No and c.file_no=b.file_No and a.file_status='Forwarded to GIS DA By Pension DA' and b.dept_forw_no=d.dept_forw_no and d.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	 ...
	 function getFilterObjection($department)
	 {
	    $q=$this->db->query("SELECT * from file_status a,pension_receipt_file_master b, pension_receipt_register_master c where a.file_no=b.file_No and a.status='Forwarded to GIS DA by GIS superintendent For Objection File' and b.dept_forw_no=c.dept_forw_no and c.branch_code=$department");
    	$result = $q->result();
 		return $result;
	 }
	...
	function get_dept_forwarding_no($file_No) 
	{
		$file_No=trim($file_No);
		$sql="SELECT * from pension_receipt_file_master where file_No='$file_No'";
		$sqls=$this->db->query($sql);
		$x=$sqls->result_array();
		if($sqls->num_rows()>0){
			return $x[0]['dept_forw_no'];
		}else{
			return 0;
		}
		
	}
	function get_district_id($dept_forwading_no) 
	{
		$dept_forwading_no=trim($dept_forwading_no);
		$sql="SELECT * from pension_receipt_register_master where dept_forw_no='$dept_forwading_no'";
		$sqls=$this->db->query($sql);
		$x=$sqls->result_array();
		if($sqls->num_rows()>0){
			return $x[0]['district'];
