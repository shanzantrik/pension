	...
	public function getOrdinaryRate()
	{
		...
		if($year >= 2006) :  //pay commission 6
			...
			if($output != 'N/A' && $output < 3500) :
				return '3500';
			elseif($output != 'N/A' && $output < 9000 && $this->class_of_pension!="Absorption_in_autonomous_body_pension") :
				return '9000';
...
else
{
	...
	public function getEarnMoney()
	{
		...
		return floor((round($pay*$this->da_percentage()/100)+$pay)*$this->earn_leave/30);
	}
	...
	public function getHalfMoney()
	{
		...
		return ceil(round((($pay+$da_amount)/2)*($this->half_leave))/30);
	}
...
//Added for View Weightage on 15/07/19 by SHantanu
	...
	{
		$check_ag=explode('s:', $this->sub_to);

		if(!empty($check_ag[1])){

		$sub_to = unserialize($this->sub_to);
