				...
				if($this->dod > $this->dor) {
					...
					if($class_of_pension == 'Normal_Family_Pension' || $class_of_pension == 'NPS' || $class_of_pension=='Death_Gratuity') :
						...
						else
						{
							$upto = new DateTime($this->dor);	
							if($detail == true) :
		          			$return.= " <b>from ".$normal_from->format('d-m-Y')." upto ".$upto2->format('d-m-Y')."</b>";
		          			endif;
						}
						return $return;
					else :
	...
	public function getDOBOfLegalHeirX($detail)//.$ci['dob']
	{
		...
		foreach ($other_info as $key => $value) {
			...
			if(empty($value['spouse_dod'])) {
				...
				//$return='';//newly added by ibad	
					//if(in_array($value['spouse_name'].">".$ci['name'], $list)) { // blocked by ibad
					if($ci['marital_status']!='legal_guardian'){
						if(empty($ci['dod'])) {
							$income = (empty($ci['income'])) ? 0 : $ci['income'];
							if($income < '3000') {
								//check his income
...
else
{
	...
	public function getNameOfLegalHeirCorrection_SingleWife($detail = true,$newsentance,$sentence_of_lifetime_arear)
	{
		$other_info = $this->family_info;
	    array_pop($other_info);
		$legal_heir = $this->family_info;
		$lharray = end($this->family_info);
		$if_legal_gur_exist=0;
		...
		foreach ($other_info as $key => $value) {
								...
								if($income < '3000') {
									...
									if($ci['marital_status']=='legal_guardian'){
										...
										if($ci['salutation'] == 'mr') {
											...
											$solutofLGurdian="Miss";
										}
										$temp_legal_gurdian=$solutofLGurdian." ".$ci['name'];

										$if_legal_gur_exist=1;
									}else{
										//check his income
										if($ci['salutation'] == 'mr') {
											$age = calculateDateDifference($ci['dob'], date('Y-m-d'));
			...
			if($if_legal_gur_exist==1){
				$return.=$newsentance.$temp_legal_gurdian;
			}else{
				$return.=$sentence_of_lifetime_arear;
			}
		}
		...
		public function getDCRG_diff_dod_dor()
	{
		$dcrg = 0;
		$latestDaAmount=0;
...
die();*/
	...
	public function getNameOfLegalHeirNameSingle($detail = true)
	{
		$other_info = $this->family_info;
	    array_pop($other_info);
		$legal_heir = $this->family_info;
	...
	public function getDOBOfLegalHeirNameSingleWife($detail = true)
	{
		$other_info = $this->family_info;
	    array_pop($other_info);
		$legal_heir = $this->family_info;
	...
	public function getIsLegalHeirCase($detail = true)
	{
		$other_info = $this->family_info;
	    array_pop($other_info);
		$legal_heir = $this->family_info;
