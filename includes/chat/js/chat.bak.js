...
function chatWith(chatuser,chatname,myname) {
...
function createChatBox(chatboxtitle,chatname,minimizeChatBox) {
	...
	.addClass("chatbox")
	.html('<div   class="chatboxhead"><div class="chatboxtitle">'+chatboxtitle+'</div><div class="chatboxoptions"><a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')">-</a> <a title="Close" href="javascript:void(0)" onclick="javascript:closeChatBox(\''+chatboxtitle+'\')">X</a></div><br clear="all"/></div><div class="chatboxcontent"></div><div class="chatboxinput"><textarea title="Write something and press enter to send your message" class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea></div>')
...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: "http://localhost/pension_ui/includes/chat/chat.php?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
		...
		jQuery.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				if (item.s == 2) {
					...
					jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+item.f+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+item.m+'<br/></span></div>');
				}
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post("http://localhost/pension_ui/includes/chat/chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post("http://localhost/pension_ui/includes/chat/chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
				message = message.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;");
				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+username+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+message+'<br/></span></div>');
				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop(jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
			});
		}
		chatHeartbeatTime = minChatHeartbeat;
...
function startChatSession(){  
	jQuery.ajax({
	  url: "http://localhost/pension_ui/includes/chat/chat.php?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
